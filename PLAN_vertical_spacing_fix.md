...
### 2. `attack_timearcs.js` — import line (~line 1)
Add `MIN_IP_SPACING` to the constants import.

### 3. `attack_timearcs.js` — final positioning (~line 2461)
Current:
```js
...
### 4. `attack_timearcs.js` — `compactIPPositions` (~line 3004)
...
### 5. `attack_timearcs.js` — brush extent (~line 1404)
...
### 6. `attack_timearcs.js` — bifocal handle chartHeight (~line 2931)
...
## Verification
1. `python -m http.server 8000`, open `attack_timearcs.html`
